...
var wcwidth$1 = wcwidth$2.exports;
...
var _require$3 = require$$0$4,
    Buffer$5 = _require$3.Buffer;
...
var node = require$$1$1.deprecate;
...
var _require$2 = state,
    getHighWaterMark$1 = _require$2.getHighWaterMark;
...
var _require$1 = state,
    getHighWaterMark = _require$1.getHighWaterMark;
	...
	constructor() {
		...
		this.ourEmit = function (event, data, ...args) {
			...
			if (self.requests > 0 || stdin.emit === self.ourEmit) {
				...
				Reflect.apply(process$2.stdin.emit, this, [event, data, ...args]);
			}
		};
	}
...
class Ora {
	...
	updateLineCount() {
		...
		for (const line of stripAnsi(' '.repeat(this.indent) + fullPrefixText + '--' + this[TEXT]).split('\n')) {
			this.lineCount += Math.max(1, Math.ceil(wcwidth$1(line) / columns));
		}
	}
	...
	start(text) {
		...
		if (this.discardStdin && process$2.stdin.isTTY) {
	...
	stop() {
		...
		if (this.discardStdin && process$2.stdin.isTTY && this.isDiscardingStdin) {
			stdinDiscarder.stop();
			this.isDiscardingStdin = false;
		}
...
var _require = require('commander'),
    program = _require.program; // const download = require('download-git-repo')
// const git = require('./git')


require('process');

var chalk = require('chalk'); // const ora = require('ora')

var inquirer = require('inquirer');

var createProject = function createProject(typeUrl, dir) {
  console.log(chalk.blueBright("creating \"".concat(dir, "\" project template...")));
  var loading = ora(chalk.blue('processing...'));
  loading.start();
  git("git clone ".concat(typeUrl, " ").concat(dir, " && cd ").concat(dir, " && rm -rf .git")).then(function () {
    loading.succeed(chalk.greenBright("created \"".concat(dir, "\" success! Enjoy Coding!")));
  })["catch"](function (err) {
    loading.fail(chalk.redBright("Error: ".concat(JSON.stringify(err))));
    process.exit(1);
  });
};

program.name('bore').command('create [project_name]').alias('c').description('create a project template').option('-t, --type [h5|mini]', 'create a specified project template').option('-g, --git <url>', 'create a project template from a git registry').action(function (cmd, cmdObj) {
  if (cmd) {
    // 有create命令-自定义创建模式
    if (cmd !== 'create' && cmd !== 'c') {
      // 命令不正确的情况下输出报错信息
      console.log(chalk.redBright("Error: no command \"".concat(cmd, "\" found! You can run \"bore -h\" to see more detail.")));
      process.exit(1);
    } else {
      // 命令正确的情况下
      ...
      if (type) {
        ...
        if (typeof type === 'boolean' && type) {
          ...
          inquirer.prompt([{
            ...
            var projectType = answer.projectType;
            var typeUrl = registry[projectType];
            var dir = args[1];
          ...
          if (typeUrl) {
            ...
            console.log(chalk.redBright("Error: no \"".concat(type, "\" type found! Choose a type from [h5|mini]")));
            process.exit(1);
          }
        }
      } else {
        ...
        if (cmdObj.git) {
          if (_dir) {
            ...
            inquirer.prompt([{
              ...
              var projectName = answer.projectName; // 未指定类型-选择类型
          ...
          if (_dir) {
            ...
            inquirer.prompt([{
              ...
              var projectType = answer.projectType;
              createProject(registry[projectType], _dir);
            });
          } else {
            // 无创建目录名
            inquirer.prompt([{
...
}).usage('[project_name] [<-t|--type> h5|mini] [[-g|--git] <url>]').helpOption('-h, --help', 'display help for bore-cli').version(chalk.gray("bore-cli ".concat(version)), '-v, --version', 'output current version of bore-cli').parse(process.argv);
