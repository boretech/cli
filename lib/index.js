...
const git = (command) => new Promise((resolve, reject) => {
...
const createProject = (typeUrl, dir) => {
    ...
    git(`git clone ${typeUrl} ${dir} && cd ${dir} && rm -rf .git`).then((res) => {
        spinner.succeed(chalk.greenBright(`${dir} created successfully! Enjoy Coding!`));
    }).catch((err) => {
...
program
    ...
    .version(chalk.gray(`bore-cli ${pkg.version}`), "-v, --version", "output current version of bore-cli")
    .parse(process.argv);
